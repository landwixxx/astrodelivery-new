    ...
    <div class="container-fluid px-6 mx-auto grid dark:text-gray-200 mb-5 pb-5">
        ...
        <div class="w-full overflow-x-auto px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            ...
            <div class="text-end mb-3">
                <a href="{{ route('painel.lojista.tipo-de-entrega.create') }}">
                    <button
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-1 py-2">Título</th>
                            <th class="px-1 py-2">Cód</th>
                            <th class="px-1 py-2">Descrição</th>
                            <th class="px-1 py-2">Tipo</sub></th>
                            <th class="px-1 py-2">Prazo Entrega</sub></th>
                            <th class="px-1 py-2">Valor</sub></th>
                            <th class="px-1 py-2">Ativo</sub></th>
                            <th class="px-1 py-2">Ações</th>

                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @if ($deliveryTypes->total() == 0)
                        ...
                        @foreach ($deliveryTypes as $type)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-1 py-2 text-truncate" style="max-width: 300px">
                                    {{ Str::limit($type->descricao, 100) ?? '-' }}
                                </td>
                                <td class="px-1 py-2">{{ $type->tipo }}</td>
                                <td class="px-1 py-2">{{ $type->tempo }}</td>
                                <td class=" px-1 py-2">{{ currency_format($type->valor) }}</td>
                                <td class=" px-1 py-2">
                                    @if ($type->ativo == 'S')
                                ...
                                <td class=" px-1 py-2">
                                    ...
                                    <a type="button" href="{{ route('painel.lojista.tipo-de-entrega.edit', $type->id) }}"
                                    ...
                                    <button class="btn btn-light border-0 dark:text-gray-200 py-0 px-1 pt-1" type="button"
                                        ...
                                        onclick="document.getElementById('form-remover').action = `{{ route('painel.lojista.tipo-de-entrega.destroy', $type->id) }}`">
