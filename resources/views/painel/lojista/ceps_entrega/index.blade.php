    ...
    <div class="container-fluid px-6 mx-auto grid dark:text-gray-200 mb-5 pb-5">
        ...
        <div class="w-full overflow-x-auto px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            ...
            <div class="text-end mb-3">
                <a href="{{ route('painel.lojista.ceps-de-entrega.create') }}">
                    <button
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-1 py-2">Tipo de entrega</th>
                            <th class="px-1 py-2">CEP Inicial</th>
                            <th class="px-1 py-2">CEP Final</sub></th>
                            <th class="px-1 py-2">Valor</sub></th>
                            <th class="px-1 py-2">Status</sub></th>
                            <th class="px-1 py-2">Ações</th>

                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @if ($deliveryZipCodes->total() == 0)
                        ...
                        @foreach ($deliveryZipCodes as $deliveryZipCode)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td scope="row" class=" px-1 py-2">
                                    {{ $deliveryZipCode->nome }}
                                </td>
                                <td scope="row" class=" px-1 py-2">
                                    {{ $deliveryZipCode->delivery_type->nome }}
                                </td>
                                <td scope="row" class=" px-1 py-2">
                                    {{ $deliveryZipCode->cep_ini }}
                                </td>
                                <td class="px-1 py-2 text-truncate" style="max-width: 300px">
                                    {{ $deliveryZipCode->cep_fim }}
                                </td>
                                <td class=" px-1 py-2">{{ currency_format($deliveryZipCode->valor) }}</td>
                                <td class=" px-1 py-2">
                                    @if ($deliveryZipCode->status == 'ativo')
                                ...
                                <td class=" px-1 py-2">
                                    ...
                                    <a type="button"
                                        href="{{ route('painel.lojista.ceps-de-entrega.edit', $deliveryZipCode->id) }}"
                                    ...
                                    <button class="btn btn-light border-0 dark:text-gray-200 py-0 px-1 pt-1" type="button"
                                        ...
                                        onclick="document.getElementById('form-remover').action = `{{ route('painel.lojista.ceps-de-entrega.destroy', $deliveryZipCode->id) }}`">
